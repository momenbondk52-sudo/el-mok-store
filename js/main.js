document.addEventListener("DOMContentLoaded", () => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            await saveData(); // حفظ الحالة المدمجة بعد تسجيل الدخول
        } else {
            currentUser = null;
    ...
    const updateUI = () => {
        ...
        if (userNav) {
            if (currentUser) {
                ...
                userNav.innerHTML = `<span style="margin-left: 1rem;">أهلاً، ${userName.split(' ')[0]}</span><a href="#" id="logout-btn">تسجيل الخروج</a>`;
