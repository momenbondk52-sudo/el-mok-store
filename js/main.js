document.addEventListener("DOMContentLoaded", () => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            // إذا كان لدى الزائر سلة، ادمجها مع سلة المستخدم المسجل
    ...
    const updateUI = () => {
        ...
        if (userNav) {
            if (currentUser) {
                // **التعديل هنا: استخدام أول اسمين فقط**
                let userName = currentUser.displayName || currentUser.email.split('@')[0];
                const nameParts = userName.split(' ');
                const displayName = nameParts.slice(0, 2).join(' '); // يأخذ أول اسمين

                userNav.innerHTML = `<span style="margin-left: 1rem;">أهلاً، ${displayName}</span><a href="#" id="logout-btn">تسجيل الخروج</a>`;
